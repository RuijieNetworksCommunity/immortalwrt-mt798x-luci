...
var callRemoveDesign = rpc.declare({
	object: 'luci.design',
	method: 'remove',
	params: ['filename'],
	expect: { '': {} }
...
var callRenameDesign = rpc.declare({
	object: 'luci.design',
	method: 'rename',
	params: ['newname'],
	expect: { '': {} }
...
var bg_path = '/www/luci-static/design/background/';
...
return view.extend({
	...
	render: function(data) {
		...
		m = new form.Map('design', _('Design theme configuration'),
			_('Here you can set the blur and transparency of the login page of design theme, and manage the background pictures and videos. Chrome is recommended.'));
		...
		o = s.option(form.Value, 'primary', _('[Light mode] Primary Color'), _('A HEX color (default: #5ea69b).'))
		o.default = '#5ea69b';
		o.rmempty = false;
		...
		o.onclick = function(ev, section_id) {
			var file = '/tmp/design_background.tmp';
			return ui.uploadFile(file, ev.target).then(function(res) {
				return L.resolveDefault(callRenameDesign(res.name), {}).then(function(ret) {
					if (ret.result === 0)
				...
				return [
					...
					E('button', {
						...
						'click': ui.createHandlerFn(this, function() {
							return L.resolveDefault(callRemoveDesign(file.name), {})
